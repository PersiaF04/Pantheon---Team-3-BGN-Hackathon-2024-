...
const DiscussionDetail = () => {
  ...
  useEffect(() => {
    const fetchDiscussion = async () => {
        try {
          const url = `${import.meta.env.VITE_BASE_URL}/discussion/${id}`;
  ...
  const handleReplySubmit = async (e) => {
    ...
    if (reply.trim()) {
      try {
        const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/discussion/${id}/reply`, { text: reply });
  ...
  return (
    <div className="flex flex-col p-4">
      ...
      <img src={discussion.thumbnail} alt={discussion.name} className="w-full h-auto object-cover mb-4" />
